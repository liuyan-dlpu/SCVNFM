function results = SCVNFMV2(X, y) % X 是特征矩阵，y 是标签向量
    % 取70%作为训练集，剩下的30%作为测试集    
    [SampleNum, ~] = size(X);
    ...
    [TrainNum, Indim] = size(x);       % the number of all samples
...
%     w = rand(Num_subset,Indim+1);       % 结论参数的初始值
    ...
    while(epochs < MaxEpochs && l == 0)
        ...
        Hidden = ones(TrainNum,Num_subset); % 隐节点初值
        Hiddensum = zeros(TrainNum,Num_subset);
        err = 0;
        ...
        %%%%%%%%%%%%%%%%%%%% 计算误差 %%%%%%%%%%%%%%
        for i = 1:TrainNum
            err = 1/TrainNum * (O(i) - NetworkOut(i)) * conj(O(i) - NetworkOut(i)) + err;
        end
        err = real(err);
        e(epochs) = err;
...
%         NetworkOut(find(NetworkOut >= 0.5)) = 1;
        ...
        rightratiotrain = rightnumber / TrainNum * 100;
        ACC(epochs) = rightratiotrain;
