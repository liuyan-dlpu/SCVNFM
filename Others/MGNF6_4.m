%%%%%%%%%%% 改编模糊推理系统实值MGNF  (0.8 - 0.5*exp(-(y(k-1))^2))*(y(k-1))-(0.3+0.9*exp(-(y(k-1))^2))*y(k-2)+0.1*sin(pi*y(k-1)) %%%%%%%%%%%%%%
    clear up
    clc
%%%%%%%%%%% 输入，输出 %%%%%%%%%%%%%%%%%%%%%%%%
Indim=2;                      %%%%输入样本维数
...
n= 0.05*randn(500,1);
for k=3:502
    y(k) = (0.8 - 0.5*exp(-(y(k-1))^2))*(y(k-1))-(0.3+0.9*exp(-(y(k-1))^2))*y(k-2)+0.1*sin(pi*y(k-1));
end
for k = 3:502;
    y1(k) = (0.8 - 0.5*exp(-(y1(k-1))^2))*(y1(k-1))-(0.3+0.9*exp(-(y1(k-1))^2))*y1(k-2)+0.1*sin(pi*y1(k-1))+n(k-2);
end
x=[y(1:500)',y(2:501)'];
O=y(3:502)';
Test1=[y1(1:500)',y1(2:501)'];
OTest=y1(3:502)';
SampleNum=size(x,1);                  %%%%输入样本个数
