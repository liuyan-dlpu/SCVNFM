% 交叉验证 %
function results = SCVNFMV1_NoMomentumL2(x, O, Test1, OTest) % X 是特征矩阵，y 是标签向量
...
%         NetworkOut(find(NetworkOut < 0.1)) = 0;
%         NetworkOut(find(NetworkOut >= 0.1 & NetworkOut < 0.3)) = 0.2;
%         NetworkOut(find(NetworkOut >= 0.3 & NetworkOut < 0.5)) = 0.4;
%         NetworkOut(find(NetworkOut >= 0.5 & NetworkOut < 0.7)) = 0.6;
%         NetworkOut(find(NetworkOut >= 0.7 & NetworkOut < 0.9)) = 0.8;
%         NetworkOut(find(NetworkOut >= 0.9)) = 1;
        ...
        %%%%%%%%%%%% 动量 %%%%%%%%%%%%%%%%%%%%
        Delte_a = Ir * Gradient_a;
        Delte_b = -Ir * Gradient_b;
        Delte_w = Ir * Gradient_w;
        a = a + Delte_a;
        b = b + Delte_b;
        w = w + Delte_w;
        ...
        TestOut111 = TestOut;
        %%%%%%%%%%%%%%%%%% 计算测试误差与准确率 %%%%%%%%%%%%%%
%         TestOut(find(TestOut < 0.1)) = 0;
%         TestOut(find(TestOut >= 0.1 & TestOut < 0.3)) = 0.2;
%         TestOut(find(TestOut >= 0.3 & TestOut < 0.5)) = 0.4;
%         TestOut(find(TestOut >= 0.5 & TestOut < 0.7)) = 0.6;
%         TestOut(find(TestOut >= 0.7 & TestOut < 0.9)) = 0.8;
%         TestOut(find(TestOut >= 0.9)) = 1;
