% 交叉验证 %
function results = SCVNFMV1_t(x, O, Test1, OTest) % X 是特征矩阵，y 是标签向量
...
%         NetworkOut(find(NetworkOut >= 0.5)) = 1;
        NetworkOut(find(NetworkOut < 0.5)) = 0;
%         NetworkOut(find(NetworkOut >= 0.5 & NetworkOut < 1.5)) = 1;
...
%         TestOut(find(TestOut >= 0.5 & TestOut < 1.5)) = 1;
...
%     sprintf('最佳测试准确率=%0.2f%% 对应训练准确率=%0.2f%%', max_rightratioTest, best_rightratiotrain
    results = [bestepoch, best_rightratiotrain, max_rightratioTest,t];
end