...
TrainMSE = zeros(ExampleNum,1);
TestMSE = zeros(ExampleNum,1);
...
while(epochs<=MaxEpochs&l==0)
   ...
   for i=1:SampleNum
     ...
     for i=1:SampleNum
         ...
         %err_mse=(O(i)-NetworkOut(i))*conj(O(i)-NetworkOut(i))+err_mse;
     end
     err;
     Traine(epochs)=err;
     Traine=real(Traine);
     trainmse = mean((O - NetworkOut').^2);
     ...
     for i=1:TestNum
         ...
         %Err_mse=(OTest(i)-TestOut(i))*conj(OTest(i)-TestOut(i))+Err_mse;
     end
     Teste(epochs)=Err;
     Teste=real(Teste);
     testmse = mean((OTest - TestOut').^2);
     %%%%%%%%%%% 测试准确率
     for TTF = 1 : TestNum
         if TestOut(TTF)<0.5
             TestOut(TTF) = 0;
...
TrainMSE(gogo,:) = train_mse;
TestMSE(gogo,:) = test_mse;
time(gogo,:) = t;
...
Mean_TrainMSE = mean(TrainMSE);
Mean_TestMSE = mean(TestMSE);
sprintf('训练准确率=%0.2f',Mean_TrainAcc)
sprintf('测试准确率=%0.2f',Mean_TestAcc)
sprintf('训练MSE=%0.2f',Mean_TrainMSE)
sprintf('测试MSE=%0.6f',Mean_TestMSE)
sprintf('平均时间=%0.6f',mean(time))
disp(Mean_Epoch);
