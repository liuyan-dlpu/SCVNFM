...
% 广义钟形隶属函数参数初始化 - 分别对实部和虚部
a_real=0.1*rand(Indim,Num_subset);               %%%%%钟形函数实部中心值
a_imag=0.1*rand(Indim,Num_subset);               %%%%%钟形函数虚部中心值
b1=0.1*rand(Indim,Num_subset);                   %%%%%钟形函数宽度参数
b2=2*rand(Indim,Num_subset);                     %%%%%钟形函数形状参数
...
Delte_b1=b1.*0;                               %%%%%宽度梯度初值
Delte_b2=b2.*0;                               %%%%%形状梯度初值
...
%%%%%%%%%%%%%%%%%%%%%%  广义钟形隶属函数定义 %%%%%%%%%%%%%%%%%%%%
% 一维广义钟形函数
bell_mf_1d = @(x_val, a_val, b1_val, b2_val) ...
    1 ./ (1 + abs((x_val - a_val) ./ max(b1_val, 1e-8)) .^ (2 * b2_val));
...
while(epochs<=MaxEpochs&l==0)
    ...
    %%%%%%%%%%%%%%%%%%%%% 重置参数 %%%%%%%%%%%%% 
     ...
     Gradient_b1=b1.*0;                               %%%%%宽度梯度初值
     Gradient_b2=b2.*0;                               %%%%%形状梯度初值
     Gradient_w=w.*0;                                  %%%%%结论参数梯度初值
     Gradient1_a_real=a_real.*0;                       %%%%%加L2实部中心梯度初值
     Gradient1_a_imag=a_imag.*0;                       %%%%%加L2虚部中心梯度初值
     Gradient1_b1=b1.*0;                              %%%%%加L2宽度梯度初值
     Gradient1_b2=b2.*0;                              %%%%%加L2形状梯度初值
   ...
   for i=1:SampleNum
         for j=1:Num_subset
             for k=1:Indim
                 ...
                 % 计算实部和虚部的钟形隶属度
                 mf_real = bell_mf_1d(x_real_val, a_real_val, b1_val, b2_val);
                 mf_imag = bell_mf_1d(x_imag_val, a_imag_val, b1_val, b2_val);
     ...
     for j=1:Num_subset
         for k=1:Indim
             for i=1:SampleNum
                 ...
                 mf_real_orig = bell_mf_1d(x_real_val, a_real_val, b1_val, b2_val);
                 mf_imag_orig = bell_mf_1d(x_imag_val, a_imag_val, b1_val, b2_val);
                 ...
                 % 对a_real的梯度
                 mf_real_pert = bell_mf_1d(x_real_val, a_real_val+epsilon, b1_val, b2_val);
                 ...
                 mf_imag_pert = bell_mf_1d(x_imag_val, a_imag_val+epsilon, b1_val, b2_val);
                 ...
                 % 对b1的梯度
                 mf_real_pert = bell_mf_1d(x_real_val, a_real_val, b1_val+epsilon, b2_val);
                 mf_imag_pert = bell_mf_1d(x_imag_val, a_imag_val, b1_val+epsilon, b2_val);
                 
                 if abs(mf_real_pert) < 1e-8, mf_real_pert = 1e-8; end
                 if abs(mf_imag_pert) < 1e-8, mf_imag_pert = 1e-8; end
                 
                 Hidden_pert = mf_real_pert * mf_imag_pert;
                 dHidden_db1 = (Hidden_pert - Hidden_orig) / epsilon;
                 
                 % 对b2的梯度
                 mf_real_pert = bell_mf_1d(x_real_val, a_real_val, b1_val, b2_val+epsilon);
                 mf_imag_pert = bell_mf_1d(x_imag_val, a_imag_val, b1_val, b2_val+epsilon);
                 
                 if abs(mf_real_pert) < 1e-8, mf_real_pert = 1e-8; end
                 if abs(mf_imag_pert) < 1e-8, mf_imag_pert = 1e-8; end
                 
                 Hidden_pert = mf_real_pert * mf_imag_pert;
                 dHidden_db2 = (Hidden_pert - Hidden_orig) / epsilon;
                 ...
                 Gradient_b1(k,j) = Gradient_b1(k,j) + ...
                     (real(Delpublic) * real(Hiddensum(i,j)) + imag(Delpublic) * imag(Hiddensum(i,j))) * dHidden_db1;
                 
                 Gradient_b2(k,j) = Gradient_b2(k,j) + ...
                     (real(Delpublic) * real(Hiddensum(i,j)) + imag(Delpublic) * imag(Hiddensum(i,j))) * dHidden_db2;
             end
